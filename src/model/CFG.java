...
public class CFG {
    ...
    public void initCYKTable(String w){
        cykTable = new String[w.length()][w.length()+1];
        for (int i = 0; i < w.length(); i++) {
            cykTable[i][0] = w.charAt(i)+"";
        }
        /*
        for (char[] chars : cykTable) {
    ...
    public ArrayList<String> searchString(String w){
    ...
    public boolean cykAlgorithm(String w) {
        ...
        for (int i = 0; i < n; i++) {
            cykTable[i][1] = StringUtils.join(searchString(cykTable[i][0]+""), ",");
        }
